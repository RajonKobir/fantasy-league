...
class TimerView extends StatefulWidget {
  ...
  const TimerView({
    ...
    this.dateStart = '',
    this.timestart = '',
    this.timerType = TimerType.ishome_screen,
    this.fontSize = 14,
...
class _TimerViewState extends State<TimerView> {
    ...
    if (finaledate!.difference(DateTime.now()).inSeconds < 0) {
      ...
      if (finaledate!.difference(DateTime.now()).inHours < 24) {
        ...
        if (date.difference(today).inDays == 1) {
          ...
          setState(() {
            timeCountDountxt =
                '${DateFormat.E().format(date)}, ${DateFormat.d().format(date)} ${DateFormat.MMM().format(date)}';
          });
        }
      }
    }
  ...
  String secondsToHoursMinutesSeconds(int seconds) {
    ...
    var finaltext = '$finalHourText : $finalMinuteText : $finalSecondText';
    if (widget.timerType == TimerType.ishome_screen) {
      finaltext = '$finalHourText : $finalMinuteText : $finalSecondText';
    ...
    return Container(
      child: Text(
        ...
        style: TextStyle(
          ...
          color: (widget.timerType == TimerType.ishome_screen)
              ? !isToday
                  ? colors
                  : HexColor('#AAAFBC')
              : colors,
