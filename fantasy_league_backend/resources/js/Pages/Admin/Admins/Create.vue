...
async function submit() {
  ...
  try {
    ...
    const response = await fetch(route('admin.admins.store'), {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      if (errorData.errors && typeof errorData.errors === 'object') {
        ...
        createError.value = errorData.message || 'Failed to create admin. Please check your inputs.'
      }
      return
    }

    // Redirect to admins list on success
    window.location.href = route('admin.admins.index')
  } catch (error) {
    createError.value = 'Failed to create admin. Please try again.'
  } finally {
