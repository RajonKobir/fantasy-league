...
// Track original admin data for change detection
...
onMounted(() => {
  if (props.admin) {
    ...
    originalData.value = {
      name: props.admin.name || '',
      email: props.admin.email || '',
      avatar_url: props.admin.avatar_url || null,
    }
    previewUrl.value = props.admin.avatar_url || null
  }
})
...
// Watch for changes in admin prop and sync form
watch(() => props.admin, (newAdmin) => {
  if (newAdmin) {
    form.value.name = newAdmin.name || ''
    form.value.email = newAdmin.email || ''
    form.value.password = ''
    form.value.password_confirmation = ''
    previewUrl.value = newAdmin.avatar_url || null
...
function onFileSelected(file) {
  ...
  previewUrl.value = file ? URL.createObjectURL(file) : props.admin?.avatar_url || null
}
...
async function submit() {
  ...
  try {
    ...
    const response = await fetch(route('admin.admins.update', props.admin.id), {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      updateError.value = 'Failed to update admin. Please check your inputs.'
      if (errorData.errors) {
    ...
    updateSuccess.value = 'Admin updated successfully.'
    ...
    updateError.value = 'Failed to update admin. Please try again.'
  } finally {
...
<template>
  ...
  <AdminLayout>
    <div class="flex justify-between items-center mb-6">
      ...
      <Link href="/admin/admins" class="px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-900">⬅ Back</Link>
    </div>
